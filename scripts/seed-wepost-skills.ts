/**
 * Seed Wepost preset skills for an account.
 *
 * Usage:
 *   npm run seed:wepost-skills
 *   npx tsx scripts/seed-wepost-skills.ts --account-id=<uuid>
 *
 * Idempotent: skips skills whose name already exists for the account.
 */

import { readFileSync, existsSync } from 'node:fs'
import { resolve } from 'node:path'
import { createClient } from '@supabase/supabase-js'
import { WEPOST_SKILLS } from '../src/lib/ai/presets/wepost'
...
async function main() {
  ...
  if (listErr) {
    console.error('Failed to list skills:', listErr.message)
    process.exit(1)
  }
  ...
  for (const skill of WEPOST_SKILLS) {
    ...
    const { error: insErr } = await db.from('ai_agent_skills').insert({
    ...
    if (insErr) {
      console.error(`Failed to insert "${skill.name}":`, insErr.message)
      process.exit(1)
    }
