...
BEGIN
  ...
  -- The BSUID index (049 / upstream 040) is the only thing stopping a
  ...
  IF to_regclass('public.idx_contacts_account_wa_user_id') IS NULL THEN
    RAISE EXCEPTION
      'idx_contacts_account_wa_user_id is missing — migration 049 did not apply';
  END IF;

  -- 050 repairs create_broadcast_with_recipients, which 043/044 shipped
  ...
  -- The failure-reason columns (051 / upstream 042) are only ever written
  ...
  IF (
    ...
    RAISE EXCEPTION
      'messages.error_code/error_title/error_details are missing — migration 051 did not apply';
  END IF;
