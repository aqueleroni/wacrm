...
import { getRoleMeta } from '@/components/settings/role-meta';
...
export default function JoinPage() {
  ...
  const handleAccept = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 409) {
          setConflictMessage(
            payload.error || t('join.conflict.defaultMessage'),
          );
        } else {
          toast.error(payload.error || t('join.toast.acceptFailed'));
        }
        setAccepting(false);
        return;
      }
      toast.success(t('join.toast.welcome'));
  ...
  const handleSignOutAndRetry = useCallback(async () => {
    ...
    try {
      ...
      toast.error(t('join.toast.signOutFailed'));
      setSigningOut(false);
    }
  }, []);
    ...
    return (
      <Card className="w-full max-w-md border-border bg-card">
        <CardContent className="flex flex-col items-center gap-3 py-12">
          ...
          <p className="text-sm text-muted-foreground">{t('join.loading')}</p>
        </CardContent>
      </Card>
    );
  }
  ...
  // ----- Peek failed -----
  if (!peek.ok) {
    const errorKey = peek.reason === 'server_error' ? 'serverError' : peek.reason === 'not_found' ? 'notFound' : peek.reason === 'used' ? 'used' : 'expired';
    return (
      <Card className="w-full max-w-md border-border bg-card">
        <CardHeader className="items-center text-center">
          ...
          <CardTitle className="text-xl text-foreground">
            {t(`join.errors.${errorKey}.title`)}
          </CardTitle>
          <CardDescription className="text-muted-foreground">
            {t(`join.errors.${errorKey}.body`)}
          </CardDescription>
        </CardHeader>
  ...
  // ----- Peek OK -----
  const expiresDate = new Date(peek.expires_at).toLocaleDateString(undefined, {
    year: 'numeric',
    month: 'short',
    day: 'numeric',
  });
  ...
  const inviteHeader = (
    <CardHeader className="items-center text-center">
      ...
      <CardTitle className="text-xl text-foreground">
        {t('join.invite.title', { account: peek.account_name })}
      </CardTitle>
      <CardDescription className="text-muted-foreground">
        {t('join.invite.role', { role: roleMetaByRole[peek.role].label })}.{' '}
        {t('join.invite.validUntil', { date: expiresDate })}
      </CardDescription>
    </CardHeader>
  );
            ...
            >
              {accepting ? (
                <>
                  ...
                  {t('join.invite.accepting')}
                </>
              ) : (
                <>
                  <CheckCircle className="size-4" />
                  {t('join.invite.accept')}
                </>
              )}
            </Button>
            <p className="text-center text-xs text-muted-foreground">
              {t('join.invite.acceptNote', { account: peek.account_name })}
            </p>
          </CardContent>
        </Card>
          ...
          <DialogContent className="bg-popover border-border sm:max-w-md">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2 text-popover-foreground">
                ...
                {t('join.conflict.title', { account: peek.account_name })}
              </DialogTitle>
            ...
            <div className="space-y-2 py-2 text-xs text-muted-foreground">
              <p>
                {t('join.conflict.instructions', { account: peek.account_name })}
              </p>
            </div>
            <DialogFooter className="bg-popover border-border">
              ...
              >
                {signingOut ? (
                  <>
                    <Loader2 className="size-4 animate-spin" />
                    {t('join.conflict.signingOut')}
                  </>
                ) : (
                  t('join.conflict.signOutAndRetry')
                )}
              </Button>
            </DialogFooter>
          </DialogContent>
      ...
      <CardContent className="flex flex-col gap-2">
        ...
        <Link href={`/login?invite=${encodeURIComponent(token!)}`}>
          <Button
            ...
            className="w-full border-border text-muted-foreground hover:bg-muted hover:text-foreground"
          >
            {t('join.invite.alreadyHaveAccount')}
          </Button>
        </Link>
      </CardContent>
    </Card>
