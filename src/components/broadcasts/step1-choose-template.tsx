...
export function Step1ChooseTemplate({ selectedTemplate, onSelect, onNext, onBack }: Step1Props) {
  ...
  useEffect(() => {
    async function fetchTemplates() {
      try {
        ...
        setError(err instanceof Error ? err.message : t('broadcasts.wizard.step1.loadFailed'));
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-semibold text-foreground">{t('broadcasts.wizard.step1.title')}</h2>
        <p className="mt-1 text-sm text-muted-foreground">
          {t('broadcasts.wizard.step1.subtitle')}
        </p>
      </div>
      ...
      {templates.length === 0 ? (
        <div className="flex h-48 flex-col items-center justify-center rounded-xl border border-border bg-card/50">
          ...
          <p className="text-sm text-muted-foreground">{t('broadcasts.wizard.step1.empty')}</p>
          <p className="mt-1 text-xs text-muted-foreground">{t('broadcasts.wizard.step1.emptyHint')}</p>
        </div>
      ) : (
