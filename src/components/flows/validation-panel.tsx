  ...
  if (issues.length === 0) {
    ...
    return (
      <div className="flex items-center gap-2 rounded-lg border border-emerald-600/50 bg-background p-3 text-sm font-medium text-emerald-300">
        ...
        {t("flows.validation.panel.ready")}
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div className="mb-2 flex items-center gap-2 text-xs text-muted-foreground">
        ...
        {t(
          errors.length === 1
            ? "flows.validation.panel.errors"
            : "flows.validation.panel.errors_plural",
          { count: errors.length },
        )}
        ,{" "}
        {t(
          warnings.length === 1
            ? "flows.validation.panel.warnings"
            : "flows.validation.panel.warnings_plural",
...
export function IssueLine({
  ...
  issue: TranslatedValidationIssue;
