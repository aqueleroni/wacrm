...
function ImportPreviewTags({
  ...
  willBeCreatedLabel: (name: string) => string;
}) {
      ...
      {tagNames.map((name) => {
        ...
        return (
          <span
            ...
            title={isKnown ? name : willBeCreatedLabel(name)}
          >
            <span
  ...
  async function handleImport() {
    ...
    try {
      ...
      if (imported > 0) {
        toast.success(
          imported === 1
            ? t('contacts.import.importedToast', { count: imported })
            : t('contacts.import.importedToast_plural', { count: imported }),
        );
        onImported();
      }
      if (tagsAssigned > 0) {
        toast.success(
          tagsAssigned === 1
            ? t('contacts.import.tagsAssignedToast', { count: tagsAssigned })
            : t('contacts.import.tagsAssignedToast_plural', { count: tagsAssigned }),
        );
      }
      ...
      if (skipped > 0) {
        toast.info(
          skipped === 1
            ...
            : t('contacts.import.duplicatesSkipped_plural', { count: skipped }),
        );
      }
      if (failed > 0) {
        toast.error(
          failed === 1
            ? t('contacts.import.importFailed', { count: failed })
            : t('contacts.import.importFailed_plural', { count: failed }),
        );
      }
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : t('contacts.import.failedGeneric');
      toast.error(message);
    } finally {
      setImporting(false);
    }
            ...
            {file ? (
              <>
                ...
                <span className="rounded-full bg-muted px-2.5 py-0.5 text-[11px] font-medium text-muted-foreground">
                  {parsedRows.length === 1
                    ? t('contacts.import.rowsReady', { count: parsedRows.length })
                    : t('contacts.import.rowsReady_plural', { count: parsedRows.length })}
                </span>
              </>
            ) : (
              <>
                          ...
                          {previewHasTags && (
                            <td className="px-3 py-2 align-top">
                              <ImportPreviewTags
                                ...
                                willBeCreatedLabel={(name) =>
                                  t('contacts.import.tagWillBeCreated', { name })
                                }
                              />
                            </td>
                          )}
              ...
              {parsedRows.length > PREVIEW_LIMIT && (
                <p className="text-center text-[11px] text-muted-foreground">
                  {parsedRows.length - PREVIEW_LIMIT === 1
                    ...
                    : t('contacts.import.moreRows_plural', {
                        count: parsedRows.length - PREVIEW_LIMIT,
                      })}
                </p>
              )}
                ...
                {result.tagsAssigned > 0 && (
                  <div className="flex items-center gap-1.5 text-sm text-cyan-400">
                    ...
                    {result.tagsAssigned === 1
                      ? t('contacts.import.tagsAssigned', { count: result.tagsAssigned })
                      : t('contacts.import.tagsAssigned_plural', {
        ...
        <DialogFooter className="mt-0 shrink-0 gap-2 border-t border-border/80 bg-background/50 px-6 py-4 sm:justify-end">
          ...
          {!result && (
            ...
            >
              ...
              {parsedRows.length === 1
                ? t('contacts.import.importButton', { count: parsedRows.length })
                : t('contacts.import.importButton_plural', { count: parsedRows.length })}
            </Button>
          )}
        </DialogFooter>
      </DialogContent>
