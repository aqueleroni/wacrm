...
export function ContactForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (phoneChanged && !parseInternationalPhone(phone)) {
      toast.error(t('contacts.form.phoneNeedsCountryCode'));
      return;
    }
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : t('contacts.form.saveFailed');
      toast.error(message);
    } finally {
      setSaving(false);
    }
      ...
      <DialogContent className="bg-popover border-border text-popover-foreground sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription className="text-muted-foreground">
            {isEdit ? t('contacts.form.editDescription') : t('contacts.form.addDescription')}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label className="text-muted-foreground">{t('contacts.table.tags')}</Label>
            {loadingTags ? (
              ...
              <p className="text-xs text-muted-foreground">
                {t('contacts.form.noTagsInSettings')}
              </p>
            ) : (
