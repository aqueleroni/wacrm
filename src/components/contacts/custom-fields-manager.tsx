...
export function CustomFieldsPanel() {
  ...
  async function handleCreate() {
    ...
    if (isDuplicate(name)) {
      toast.error(t('contacts.customFields.duplicate', { name }));
      return;
    }
    ...
    if (error) {
      toast.error(t('contacts.customFields.createFailed'));
      return;
    }
  ...
  async function handleRename(
    ...
    setBusyId(null);
    if (error) {
      toast.error(t('contacts.customFields.renameFailed'));
      return false;
    }
    await fetchFields();
    return true;
  ...
  async function handleDelete(field: CustomField) {
    if (
      !window.confirm(
        t('contacts.customFields.deleteConfirm', { name: field.field_name }),
      )
    ) {
      return;
    }
    ...
    setBusyId(null);
    if (error) {
      toast.error(t('contacts.customFields.deleteFailed'));
      return;
    }
  ...
  return (
    <li className="flex items-center gap-2 px-3 py-2">
      ...
      <Button
        ...
        title={t('contacts.customFields.deleteField')}
