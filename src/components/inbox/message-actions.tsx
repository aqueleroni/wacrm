...
export function MessageActions({
  ...
  const handleCopy = async () => {
    ...
    if (!text) {
      toast.error(t("inbox.messages.nothingToCopy"));
      return;
    }
    try {
      await navigator.clipboard.writeText(text);
      toast.success(t("inbox.messages.copied"));
    } catch {
      ...
      >
        <Popover open={pickerOpen} onOpenChange={setPickerOpen}>
          <PopoverTrigger
            ...
            aria-label={t("inbox.messages.react")}
          >
          ...
          >
            {QUICK_EMOJIS.map((e) => (
              <button
                ...
                aria-label={t("inbox.messages.reactWith", { emoji: e })}
              >
                {e}
              </button>
            ))}
        ...
        <button
          ...
          onClick={handleReply}
          className="flex h-5 w-5 items-center justify-center rounded-full text-popover-foreground hover:bg-muted hover:text-foreground"
          aria-label={t("inbox.messages.reply")}
        >
        ...
        <button
          ...
          onClick={handleCopy}
          className="flex h-5 w-5 items-center justify-center rounded-full text-popover-foreground hover:bg-muted hover:text-foreground"
          aria-label={t("inbox.messages.copy")}
        >
