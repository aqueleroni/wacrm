...
export function ProfileForm() {
  ...
  const onPickFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!ALLOWED_MIME.has(file.type)) {
      toast.error(t('settings.profile.errors.unsupportedImage'), {
        description: t('settings.profile.errors.unsupportedImageHint'),
      });
      return;
    }
    if (file.size > MAX_AVATAR_BYTES) {
      toast.error(t('settings.profile.errors.imageTooLarge'), {
        description: t('settings.profile.errors.imageTooLargeHint'),
      });
      return;
    }
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (!trimmedName) {
      toast.error(t('settings.profile.errors.displayNameRequired'));
      return;
    }
    const trimmedEmail = email.trim();
    if (!EMAIL_RE.test(trimmedEmail)) {
      toast.error(t('settings.profile.errors.invalidEmail'));
      return;
    }
    ...
    try {
      ...
      if (trimmedEmail.toLowerCase() !== profile.email.toLowerCase()) {
        ...
        if (emailError) {
          ...
          toast.success(t('settings.profile.success.saved'));
          toast.error(
            t('settings.profile.errors.emailChangeFailed', {
      ...
      toast.success(
        emailSent
          ? t('settings.profile.success.savedEmailPending')
          : t('settings.profile.success.saved'),
      );
    } catch (err) {
        ...
        <Card>
          ...
          <div className="flex flex-wrap items-center gap-5">
            <Avatar size="lg" className="size-16">
              {currentAvatar ? (
                <AvatarImage src={currentAvatar} alt={fullName || t('common.misc.avatar')} />
              ) : null}
          ...
          <div className="space-y-2">
            ...
            {emailChangePending && (
              <p className="flex items-start gap-2 rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-700 dark:text-amber-300">
                ...
                <span>
                  {t('settings.profile.emailConfirmBoth', {
