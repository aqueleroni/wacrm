...
export function ApiKeysSettings() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(payload.error || t('settings.api.create.errors.loadFailed'));
        return;
      }
  ...
  async function handleRevoke(key: ApiKey) {
    ...
    try {
      ...
      if (!res.ok) {
        const payload = await res.json().catch(() => ({}));
        toast.error(payload.error || t('settings.api.create.errors.revokeFailed'));
        return;
      }
      toast.success(t('settings.api.revokedToast', { name: key.name }));
  ...
  return (
    <section className="animate-in fade-in-50 space-y-6 duration-200">
      <SettingsPanelHead
        ...
        description={
          <>
            {t('settings.api.descriptionRich', {
              path: '/api/v1',
          ...
          <CardContent className="flex flex-col items-center justify-center py-10 text-center">
            ...
            {canEditSettings ? (
              <p className="text-muted-foreground mt-1 text-xs">
                {t('settings.api.emptyAdmin')}
              </p>
            ) : (
              <p className="text-muted-foreground mt-1 text-xs">
                {t('settings.api.emptyViewer')}
              </p>
            )}
          </CardContent>
        </Card>
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-muted-foreground mt-1.5 text-xs">
                        {t('settings.api.created', { date: fmtDate(k.created_at) })}
                        {' · '}
                        {k.last_used_at
                          ? t('settings.api.lastUsed', {
                              date: fmtDate(k.last_used_at),
                            })
                          : t('settings.api.neverUsed')}
                        {k.expires_at && status !== 'expired'
                          ? ` · ${t('settings.api.expires', { date: fmtDate(k.expires_at) })}`
                          : ''}
                      </p>
                    </div>
...
function CreateKeyDialog({
  ...
  async function handleCreate() {
    ...
    if (!trimmed) {
      toast.error(t('settings.api.create.errors.nameRequired'));
      return;
    }
    setSubmitting(true);
    try {
      ...
      const payload = await res.json().catch(() => ({}));
      if (!res.ok) {
        toast.error(payload.error || t('settings.api.create.errors.createFailed'));
        return;
      }
  ...
  async function copyKey() {
    ...
    try {
      ...
      toast.success(t('settings.api.create.success.copied'));
    } catch {
      toast.error(t('settings.api.create.errors.copyManual'));
    }
  }
      ...
      <DialogContent className="border-border bg-popover sm:max-w-md">
        {createdKey ? (
          <>
            <DialogHeader>
              <DialogTitle className="text-popover-foreground">
                {t('settings.api.create.revealTitle')}
              </DialogTitle>
              <DialogDescription className="text-muted-foreground">
                {t('settings.api.create.revealDescription')}
              </DialogDescription>
            </DialogHeader>

            <div className="space-y-1.5">
              <Label className="text-muted-foreground">
                {t('settings.api.create.keyLabel')}
              </Label>
          ...
          <>
            <DialogHeader>
              <DialogTitle className="text-popover-foreground">
                {t('settings.api.create.title')}
              </DialogTitle>
              <DialogDescription className="text-muted-foreground">
                {t('settings.api.create.description')}
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-4">
              <div className="space-y-1.5">
                <Label htmlFor="api-key-name" className="text-muted-foreground">
                  {t('settings.api.create.name')}
                </Label>
                <Input
                  id="api-key-name"
                  value={name}
                  maxLength={80}
                  placeholder={t('settings.api.create.namePlaceholder')}
              ...
              <div className="space-y-2">
                    ...
                    >
                      ...
                      <span className="min-w-0">
                        ...
                        <span className="text-muted-foreground block text-xs">
                          {t(
                            `settings.api.create.scopeDescriptions.${scope}`,
                          )}
                        </span>
                      </span>
                    </label>
                ...
                <p className="text-muted-foreground text-xs">
                  {t('settings.api.create.noScopesHint')}
                </p>
              </div>
            </div>
